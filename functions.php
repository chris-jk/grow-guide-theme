        ...
        ?>
            <style type="text/css">
                :root {
                    ...
                    ;
                    --accent-secondary:
                        <?php echo esc_attr(adjust_brightness($accent_color, -20));
                        ?>
                    ;
                    --accent-hover:
                        <?php echo esc_attr(adjust_brightness($accent_color, 20));
...
/**
 * Grow Guide Theme functions and definitions
 */
...
{
    // From generic_theme_setup
    add_theme_support('post-thumbnails'); // Already in grow_guide_setup, ensure no duplication
    add_theme_support('automatic-feed-links'); // Already in grow_guide_setup
    add_theme_support('title-tag'); // Already in grow_guide_setup
    add_theme_support('html5', array(
        'search-form',
        'comment-form',
        'comment-list', // This was mentioned in your selection
        'gallery',
        'caption',
        // 'style', // from grow_guide_setup
        // 'script', // from grow_guide_setup
    ));
    // Ensure the HTML5 array is merged correctly if there are differences.
    // The grow_guide_setup also had 'style' and 'script' for HTML5 support.

    // Register navigation menu (ensure all desired menus are registered here)
    register_nav_menus(array(
        'primary' => esc_html__('Primary Menu', 'grow-guide'), // Using text domain from grow_guide
        'footer-menu' => esc_html__('Footer Menu', 'grow-guide'), // from grow_guide_setup
    ));


    // From grow_guide_setup (original parts)
    ...
    // From generic_custom_image_sizes (if you keep that logic here)
    add_image_size('archive-thumbnail', 400, 200, true);
    add_image_size('hero-image', 1200, 600, true);
}
add_action('after_setup_theme', 'grow_guide_setup');

// REMOVE the generic_theme_setup function and its add_action call.
// function generic_theme_setup() { ... }
// add_action('after_setup_theme', 'generic_theme_setup');
 ...
 * Consolidate from generic_widgets_init and grow_guide_widgets_init
 */
function grow_guide_widgets_init()
{
    // From generic_widgets_init
    register_sidebar(array(
        'name' => esc_html__('Sidebar', 'grow-guide'), // Using text domain
        'id' => 'sidebar-1', // Ensure IDs are unique if merging
        'description' => esc_html__('Add widgets here to appear in your sidebar.', 'grow-guide'),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget' => '</section>',
        'before_title' => '<h3 class="widget-title">', // Changed from h2 to h3 for consistency if needed
        'after_title' => '</h3>',
    ));

    // From generic_widgets_init (Footer widget)
    register_sidebar(array(
        'name' => esc_html__('Footer', 'grow-guide'),
        'id' => 'footer-1',
        'description' => esc_html__('Add widgets here to appear in your footer.', 'grow-guide'),
        'before_widget' => '<div id="%1$s" class="footer-widget %2$s">',
        'after_widget' => '</div>',
        'before_title' => '<h4 class="footer-widget-title">',
        'after_title' => '</h4>',
...
// REMOVE the generic_widgets_init function and its add_action call.
// function generic_widgets_init() { ... }
// add_action('widgets_init', 'generic_widgets_init');
 ...
 * Consolidate from generic_theme_scripts and grow_guide_scripts
 */
function grow_guide_scripts()
{
    // From generic_theme_scripts
    wp_enqueue_style('generic-style', get_stylesheet_uri(), array(), '1.0'); // Consider renaming to grow-guide-style
    wp_enqueue_style('google-fonts', 'https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&display=swap', array(), null);

    $unified_scripts_path = get_template_directory() . '/assets/js/unified-scripts.js';
    $app_links_path = get_template_directory() . '/app-links.js';

    $script_handle_to_localize = '';

    if (file_exists($unified_scripts_path)) {
        wp_enqueue_script('grow-guide-unified-scripts', get_template_directory_uri() . '/assets/js/unified-scripts.js', array('jquery'), _S_VERSION, true); // Added jquery dependency, updated version
        $script_handle_to_localize = 'grow-guide-unified-scripts';
    } elseif (file_exists($app_links_path)) {
        wp_enqueue_script('grow-guide-app-links', get_template_directory_uri() . '/app-links.js', array('jquery'), _S_VERSION, true); // Added jquery dependency, updated version
        $script_handle_to_localize = 'grow-guide-app-links';
    }

    if (is_front_page() && !empty($script_handle_to_localize)) {
        $app_name = get_theme_mod('app_name', 'Grow Guide');
        $app_store_url = get_theme_mod('app_store_url', 'https://apps.apple.com/us/app/grow-guide/id6637720578');
        $google_play_url = get_theme_mod('google_play_url', 'https://play.google.com/store/apps/details?id=app.growguide');
        $google_play_id = get_theme_mod('google_play_id', 'com.growguide'); // Corrected from app.growguide
        $apple_app_id = get_theme_mod('apple_app_id', '');

        wp_localize_script($script_handle_to_localize, 'appUrls', array(
            'appName' => $app_name,
            'appStore' => $app_store_url,
            'googlePlay' => $google_play_url,
            'googlePlayId' => $google_play_id,
    ...
    // From grow_guide_scripts (original parts, if any, or add your main theme JS)
    // wp_enqueue_style('grow-guide-custom-style', get_template_directory_uri() . '/assets/css/custom.css', array(), _S_VERSION);
    // wp_enqueue_script('grow-guide-custom-js', get_template_directory_uri() . '/assets/js/custom.js', array('jquery'), _S_VERSION, true);
...
// REMOVE the generic_theme_scripts function and its add_action call.
// function generic_theme_scripts() { ... }
// add_action('wp_enqueue_scripts', 'generic_theme_scripts');


// ... other functions from the generic section that you want to keep, ensure they are uniquely named or integrated ...

// ... existing code ...
// The rest of your generic_ functions like generic_customize_register, get_default_feature_title, etc.
// You'll need to decide if these should be renamed (e.g., grow_guide_customize_register) or kept as generic_
// if they are truly generic helper functions.
// For Customizer functions, it's common to prefix them with the theme name.

// For example, generic_color_customize_register and generic_custom_colors
// could become grow_guide_color_customize_register and grow_guide_custom_colors.
// Make sure to update the add_action calls accordingly.

// The helper functions like adjust_brightness, generic_estimated_reading_time, generic_get_category_icon
// can remain as they are if their names are descriptive enough and don't conflict.

// The meta box functions (generic_add_featured_post_meta_box, etc.) and
// generic_create_learn_page can also be kept or renamed for consistency.

/**
 * Custom template tags for this theme.
 */
// require get_template_directory() . '/inc/template-tags.php'; // Ensure these files exist

/**
 * Functions which enhance the theme by hooking into WordPress.
 */
// require get_template_directory() . '/inc/template-functions.php'; // Ensure these files exist

/**
 * Customizer additions.
 */
// require get_template_directory() . '/inc/customizer.php'; // Ensure these files exist and don't conflict with generic_customize_register
